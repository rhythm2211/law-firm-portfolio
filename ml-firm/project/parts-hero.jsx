...
function LiveClock() {
  ...
  return (
    <span className="font-mono text-paper/60">London · {time}</span>
  );
}

// ============ Navbar ============
function Navbar({ onBook, onChat }) {
  const links = [
    ["Practice", "#practice"],
    ["Cases", "#cases"],
    ["People", "#people"],
...
function MeridianMark({ className = "h-7 w-7" }) {
  return (
...
function Hero({ onBook, onChat }) {
  return (
    <section id="top" className="relative min-h-[100svh] text-paper overflow-hidden">
      ...
      {/* faint courthouse imagery */}
      <SafeImg
        src={window.LAW_IMG.courthouseColumns}
        alt=""
        ...
        <div className="mt-12 grid grid-cols-1 lg:grid-cols-[1.2fr_1fr] gap-10 items-end">
          ...
          <div className="flex flex-col sm:flex-row gap-4">
            ...
            >
              ...
              <span className="text-[10px] font-mono text-paper/60 border border-white/15 rounded px-1.5 py-[1px]">24/7</span>
            </button>
          </div>
        </div>
...
// ============ Trusted by / Recognition ============
function Recognition() {
  const items = [...window.RECOGNITION, ...window.RECOGNITION];
  return (
      ...
      <div className="relative">
        <div className="marquee">
          {items.map((label, i) => (
            <div
              key={i}
              className="flex items-center gap-4 text-paper/85 whitespace-nowrap"
            >
...
Object.assign(window, { AnnouncementBar, Navbar, Hero, Recognition, MeridianMark });
